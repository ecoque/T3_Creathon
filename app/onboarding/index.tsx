...
const ROLES: { id: ParticipantRole; descriptionKey: string; fallbackDescription: string; Icon: typeof Rocket }[] = [
  { id: 'girisimci', descriptionKey: 'onboarding.roleGirisimciDesc', fallbackDescription: 'Girişimini tanıt, yatırımcı ve iş ortağı bul.', Icon: Rocket },
  { id: 'yatirimci', descriptionKey: 'onboarding.roleYatirimciDesc', fallbackDescription: 'Yeni girişimleri keşfet ve yatırım fırsatlarını değerlendir.', Icon: BriefcaseBusiness },
  { id: 'kurum', descriptionKey: 'onboarding.roleKurumDesc', fallbackDescription: 'İş birliği ve açık inovasyon fırsatlarını takip et.', Icon: Building2 },
  { id: 'ziyaretci', descriptionKey: 'onboarding.roleZiyaretciDesc', fallbackDescription: 'Programı keşfet, topluluğa dahil ol.', Icon: Users },
];
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top', 'bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.eyebrow}>{t('onboarding.eyebrow', 'TAKEOFF İSTANBUL 2026')}</Text>
        <Text style={styles.title}>{t('onboarding.roleTitle')}</Text>
        <Text style={styles.subtitle}>{t('onboarding.roleSubtitle', 'Sana daha uygun kişiler ve içerikler önermemize yardımcı olur.')}</Text>

        <View style={styles.roleList}>
          {ROLES.map(({ id, descriptionKey, fallbackDescription, Icon }) => {
            ...
            return (
              ...
              >
                <View style={[styles.iconCircle, selected && styles.iconCircleSelected]}>
                  <Icon size={20} color={selected ? colors.white : colors.primary} strokeWidth={2.2} />
                </View>
                <View style={styles.cardBody}>
                  <Text style={styles.cardTitle}>{t(ROLE_LABEL_KEY[id])}</Text>
                  <Text style={styles.cardDescription}>{t(descriptionKey, fallbackDescription)}</Text>
                </View>
        ...
        <Text style={styles.note}>{t('onboarding.roleNote', 'Bu seçimleri daha sonra profilinden güncelleyebilirsin.')}</Text>
